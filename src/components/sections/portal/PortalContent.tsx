...
function LinkCard({
  ...
  const cls =
    'flex items-start gap-3.5 p-4 bg-white border border-gray-200 rounded-xl hover:border-brand-navy hover:shadow-md transition-all group';

  const inner = (
    <>
      <div
        className="w-10 h-10 bg-brand-navy/10 group-hover:bg-brand-navy rounded-xl flex items-center justify-center flex-shrink-0 transition-colors"
        aria-hidden="true"
      >
        <Icon size={18} className="text-brand-navy group-hover:text-brand-gold transition-colors" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-gray-900 text-sm leading-snug group-hover:text-brand-navy transition-colors flex items-center gap-1.5">
          {label}
          {external && href !== '#' && (
      ...
      <ChevronRight
        size={14}
        className="text-gray-300 group-hover:text-brand-navy flex-shrink-0 mt-1 transition-colors"
        aria-hidden="true"
      />
    </>
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <section>
          <div className="flex items-center gap-3 mb-5">
            <div className="w-10 h-10 bg-brand-navy rounded-xl flex items-center justify-center" aria-hidden="true">
              <GraduationCap size={20} className="text-brand-gold" />
            </div>
            <div>
              <h2 className="font-bold text-brand-navy text-lg leading-tight">Portal & Tautan Sistem</h2>
